...
Critter::Critter() 
: name("default_critter"), hunger(0), boredom(0), height(5), thirst(hunger)
{
	cout << "Default constructor called" << endl;
}

Critter::Critter(string newname) 
: name(newname), hunger(0), boredom(0), height(5), thirst(hunger)
{
	cout << "Constructor with only name as a parameter" << endl;
}

Critter::Critter(string new_n, int new_hu, int new_b, double new_he) 
: name(new_n), hunger(convert_todouble(new_hu)), boredom(new_b),
height(new_he), thirst(hunger)
{
	cout << "Constructor with 4 parameters" << endl;
}

Critter::Critter(string n, int hu, int b, double he, double th) 
: name(n), hunger(convert_todouble(hu)), boredom(b), 
height(he), thirst(th)
{
...
void Critter::print() {
	...
	<< getHeight() << ". My thirst level is " << getThirst()
	<< "." << endl;
}
